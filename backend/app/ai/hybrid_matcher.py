from app.ai.semantic_matcher import SemanticMatcher
from app.repositories.job_repository import JobRepository
from app.repositories.job_skill_repository import JobSkillRepository
from app.repositories.resume_repository import ResumeRepository
from app.repositories.resume_skill_repository import ResumeSkillRepository


class HybridMatcher:

    def __init__(self, db):
        self.db = db
        self.semantic = SemanticMatcher()
        self.job_repo = JobRepository(db)
        self.resume_repo = ResumeRepository(db)
        self.job_skill_repo = JobSkillRepository(db)
        self.resume_skill_repo = ResumeSkillRepository(db)

    # --------------------------------------------------
    # Existing Ranking
    # --------------------------------------------------
    def rank_candidates(
        self,
        job,
    ):
        semantic_results = self.semantic.search(
            job.description,
            limit=25,
        )

        job_skills = set(self.job_skill_repo.get_skills(job.id))
        ranked = []

        for candidate in semantic_results:
            resume_skills = set(
                self.resume_skill_repo.get_skills(candidate["resume_id"])
            )

            # Use intersection for matched skills
            matched = job_skills.intersection(resume_skills)

            skill_score = (
                (len(matched) / len(job_skills)) * 100 if job_skills else 0
            )

            completeness = min(len(resume_skills) * 5, 100)

            overall_score = round(
                (
                    candidate["semantic_score"] * 0.60
                    + skill_score * 0.30
                    + completeness * 0.10
                ),
                2,
            )

            ranked.append(
                {
                    "candidate_id": candidate["candidate_id"],
                    "resume_id": candidate["resume_id"],
                    "semantic_score": round(candidate["semantic_score"], 2),
                    "skill_score": round(skill_score, 2),
                    "resume_completeness": completeness,
                    "overall_score": overall_score,
                    "matched_skills": sorted(matched),
                    # Use difference for missing skills
                    "missing_skills": sorted(
                        job_skills.difference(resume_skills)
                    ),
                }
            )

        ranked.sort(key=lambda x: x["overall_score"], reverse=True)
        return ranked

    # --------------------------------------------------
    # Single Candidate Match
    # --------------------------------------------------
    def match_candidate(
        self,
        job_id: int,
        resume_id: int,
    ):
        job = self.job_repo.get_by_id(job_id)

        resume_skills = set(self.resume_skill_repo.get_skills(resume_id))
        job_skills = set(self.job_skill_repo.get_skills(job_id))

        # Use intersection for matched skills
        matched = job_skills.intersection(resume_skills)

        skill_score = (len(matched) / len(job_skills)) * 100 if job_skills else 0
        completeness = min(len(resume_skills) * 5, 100)

        semantic_results = self.semantic.search(job.description, limit=100)

        semantic_score = 0
        candidate_id = None

        for result in semantic_results:
            if result["resume_id"] == resume_id:
                semantic_score = result["semantic_score"]
                candidate_id = result["candidate_id"]
                break

        overall_score = round(
            (
                semantic_score * 0.60
                + skill_score * 0.30
                + completeness * 0.10
            ),
            2,
        )

        return {
            "candidate_id": candidate_id,
            "resume_id": resume_id,
            "semantic_score": round(semantic_score, 2),
            "skill_score": round(skill_score, 2),
            "resume_completeness": completeness,
            "overall_score": overall_score,
            "matched_skills": sorted(matched),
            # Use difference for missing skills
            "missing_skills": sorted(
                job_skills.difference(resume_skills)
            ),
        }
